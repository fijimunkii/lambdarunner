...
const lambda = new AWS.Lambda({
  region: env.get('aws-region') || 'us-east-1',
	...(env.get('AWS_ENDPOINT') ? { endpoint: new AWS.Endpoint(env.get('AWS_ENDPOINT')) } : {})
});
...
module.exports = async (FunctionName, Qualifier) => {
  ...
  const dir = path.normalize(path.join(env.get('DATA_DIR')||os.tmpdir(),FunctionName,Qualifier));
  ...
  if (doesNotExist) {
    ...
    await new Promise((resolve, reject) => {
      if (env.get('AWS_ENDPOINT')) { // fix for bad download url from localstack
        lambdaFn.Code.Location = lambdaFn.Code.Location.replace(/\?.*/, '/code');
      }
      const get = /^https/.test(lambdaFn.Code.Location) ? https.get : http.get;
      get(lambdaFn.Code.Location, res => {
        res
    ...
    await mkdirp(path.join(dir,'node_modules'));
    ...
    await Promise.delay(10); // filesystem race condition
  }
